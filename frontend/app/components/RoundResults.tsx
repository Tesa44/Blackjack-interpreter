...
const CardDisplay: React.FC<{ cards: Card[] }> = ({ cards }) => (
  <div className="flex flex-wrap gap-1">
    {cards.map((card, index) => (
      <span key={index} className="inline-block bg-white border border-gray-300 rounded px-2 py-1 text-sm font-mono">
  ...
  <div className="bg-white rounded-lg shadow-md p-4 mb-4 border-l-4 border-blue-500">
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      <div>
        ...
        <p className="text-sm text-gray-600">Result: <span className={`font-medium ${round.result === 'PLAYER_WIN' ? 'text-green-600' : round.result === 'DEALER_WIN' ? 'text-red-600' : 'text-yellow-600'}`}>{round.result}</span></p>
        <p className="text-sm text-gray-600">Action: {round.action}</p>
        <p className="text-sm text-gray-600">Net Bet: {round.netBetUnits} units</p>
      </div>
      <div>
        <h5 className="font-medium text-gray-700">Dealer</h5>
        <p className="text-sm">Total: {round.dealerTotal}</p>
        <p className="text-sm">Upcard: {round.dealerUpcard}</p>
        <CardDisplay cards={round.dealerCards} />
      </div>
      <div>
        <h5 className="font-medium text-gray-700">Player</h5>
        <p className="text-sm">Initial Total: {round.playerInitialTotal}</p>
        {round.playerHands.map((hand: PlayerHand, handIndex: number) => (
          <div key={handIndex} className="mt-2">
            <p className="text-sm">Hand {handIndex + 1}: {hand.bestValue} {hand.isSoft ? '(Soft)' : ''} {hand.isPair ? '(Pair)' : ''}</p>
...
const RoundResults: React.FC<RoundResultsProps> = ({ showResults }) => {
  ...
  return (
    <div className="space-y-6 overflow-auto max-h-96">
